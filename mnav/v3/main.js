...
var main = (function () {
	...
	function init() {
		...
		$('#bottom-bar').append(bottomBar.getEl());
	...
	function hasScrolled() {
		...
		if (!teamMenuHeight) {
			teamMenuHeight = $('#nav-menu-team').outerHeight();
		}

		if (lastScrollTop - st > 0){
			$('#nav-menu-team').css({bottom: 0});
		} else {
			$('#nav-menu-team').css({bottom: `${-teamMenuHeight}px`});
		}
	...
	function initTeamMenu() {
		$(document.body).append(`
			<div id="nav-menu-team" class="nav-menu-team">
				<div class="title">
					<div class="team-avatar">C</div>
					<div class="text">Customer Feedback team</div>
					<div class="settings"></div>
				</div>
				<div class="user-list" id="team-members"></div>
				<div id="team-items" class="menu-items"></div>
			</div>
		`);
	}
